...
def query_api(api_data, hostname="localhost", port=8001, verbose=False, debug=False):
    ...
    for api_entry in api_data:
        match api_entry['api']:
                            ...
                            for document in data["data"]:  # Iterate through the data
                                if document["doc_metadata"]["file_name"] == params["file"]:
                                    ...
                                    with requests.delete(url, headers=headers, verify=False) as delete_response:
                                        print('Delete Request: status', delete_response)
                    end_time = time.time()  # Record end time
                    elapsed_time = end_time - start_time
                    results.append({"prompt": params["description"], "time_elapsed": elapsed_time})

            case 'generator/docx_template':

                for params in api_entry['queries']:
                    doc = DocxTemplate(params['template_docx'])
                    context = {}
                    for result in results:
                        json_acceptable_string = result['full_response'].replace("'", "\"")
                        context = json.loads(json_acceptable_string)
                        print("Context ",context)
                        doc.render(context)
                    doc.save(params['output_filename'])

            case 'output/docx_template':
                for params in api_entry['queries']:
                    doc = DocxTemplate(params['template_docx'])
                    context = {}
                    for result in results:
            ...
            case 'output/docx':
            #
            # Output whatever is in results to Docx
                for params in api_entry['queries']:
                        doc = docx.Document()
                        # Add a new paragraph with the provided content
                        if results:
                            for result in results:
                                if 'user_message' in result and 'full_response' in result:
                                    doc.add_heading(result['user_message'])
                                    doc.add_paragraph(result['full_response'])

                            # Save the document with the given file name
                            doc.save(params['file_name'])
